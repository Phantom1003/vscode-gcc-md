...
/* Return TRUE if pec return address protection is enable */
bool 
riscv_pec_return_address_protection (void) 
{
  return (riscv_pec_ra_mode == RISCV_PEC_RA_ALL ||
	        (riscv_pec_ra_mode == RISCV_PEC_RA_NOLEAF
  ...
  if ((frame->mask | frame->fmask) != 0)
    {
      ...
      /* Pointer Encryption Extension */
      if (riscv_pec_return_address_protection()) {
        rtx ra = gen_rtx_REG (Pmode, RETURN_ADDR_REGNUM);
        insn = gen_cretkdi3 (ra, ra, stack_pointer_rtx);
        emit_insn (insn);
      }
...
riscv_expand_epilogue (int style)
{
  ...
  /* Pointer Encryption Extension */
  if (riscv_pec_return_address_protection()) {
    insn = gen_crdtkdi3 (ra, ra, stack_pointer_rtx);
    emit_insn (insn);
  }
